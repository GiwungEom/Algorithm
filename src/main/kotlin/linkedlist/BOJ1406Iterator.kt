...
fun main() {
    ...
    val textLinkedList = LinkedList<Char>()
    bufferedReader.readLine().toCharArray().forEach {
        textLinkedList.add(it)
    }

    val m = bufferedReader.readLine().toInt()
    val cursor = textLinkedList.listIterator(textLinkedList.lastIndex)
    ...
    repeat(m) {
        ...
        val tokenizer = StringTokenizer(commands)
        when (tokenizer.nextToken()) {
            ...
            "P" -> {
                cursor.add(tokenizer.nextToken().toCharArray()[0])
            }
            else -> throw IllegalStateException("Unknown Command")
        }
    }
    bufferedWriter.append(textLinkedList.joinToString(""))
