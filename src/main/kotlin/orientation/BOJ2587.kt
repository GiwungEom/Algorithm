...
fun main(arg: Array<String>) {
    ...
    repeat(5) {
        linkedList.add(bufferedReader.readLine().toInt())
    }
    linkedList.sort()
    val avg = linkedList.average().toInt().toString()
    val mid = linkedList[linkedList.size / 2].toString()
    with(bufferedWriter) {
        append(avg).appendLine()
        append(mid).appendLine()
        flush()
        close()
    }
