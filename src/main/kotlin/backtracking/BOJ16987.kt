 ...
 * 어떻게 하면 계란을 조심스럽게 깰 수 있을까 고민하던 인범이에게 유현이는 굉장히 좋은 해결책을 알려주었다. 바로 계란으로 계란을 치는 것이다.
...
fun main() {
    ...
    val board = Array(size = n) { Pair(0, 0) }
    var count = 0

    repeat(n) {
        val tokenizer = StringTokenizer(bufferedReader.readLine())
        board[it] = tokenizer.nextToken().toInt() to tokenizer.nextToken().toInt()
    }
    ...
    fun func(k: Int) {
        ...
        if (board[k].first <= 0 || count == n - 1) {
            func(k + 1)
            return
        }

        for (i in 0 until n) {
            if (k == i || board[i].first <= 0) continue
            val selectedEgg = board[k]
            val nextEgg = board[i]

            val leftEgg = selectedEgg.first - nextEgg.second to selectedEgg.second
            val rightEgg = nextEgg.first - selectedEgg.second to nextEgg.second

            board[k] = leftEgg
            board[i] = rightEgg

            if (leftEgg.first <= 0) count ++
            if (rightEgg.first <= 0) count ++
    ...
    bufferedWriter.append(maxCount.toString())
